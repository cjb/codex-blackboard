...
useful = (node) ->
  return false unless node instanceof Element
...
assignMessageFollowup = (curr, prev=undefined) ->
  ...
  # use bb-message-followup-client instead of bb-message-followup, since
  # the client-side rendering can "see through" hidden useless non-.media
  # nodes.
...
refreshFollowups = ->
  return unless model.followupStyle() is 'client'
